...
class Model(object):
    ...
    def build_graph(self, hparams):
        ...
        emb_inp_v2={}
        for s in hparams.single_features:
            if s not in self.f1 and s not in self.f2:
                continue
            else:
        ...
        b_1 = tf.get_variable("b_1", [1, hparams.hidden_size[0]],initializer=initializer)
        ...
        b_2 =tf.get_variable("b_2", [1, hparams.hidden_size[1]],initializer=initializer)
...
def train(hparams):
        ...
        if hparams.mode!='train':
            ...
            while True:
                    ...
                    if global_step%hparams.num_display_steps==0 or global_step>=hparams.epoch:
                            ...
                            if global_step%hparams.num_eval_steps==0 or global_step>=hparams.epoch:
                                epoch=False
                                preds=[]
                                label=[]
                                aid=[]
        ...
        #df.to_csv('submission_dev_'+str(hparams.sub_name)+'.csv',index=False)
