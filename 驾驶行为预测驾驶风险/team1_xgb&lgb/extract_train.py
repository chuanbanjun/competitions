...
def extrac_train_time_feature(df,column_name="TIME"):
...
def get_train_feature(df,userId,feature_df,sampleRate=0.5):
    ...
    for userId in userId:
        ...
        feature_df.loc[userId, "Y"] = user["Y"][0]

    del data
    gc.collect()
    print("extrac feature  is ok")

def extracTrainFea(df, sampleRate=0.5):
    ...
    extrac_train_time_feature(df)
    get_train_feature(df, userId,feature_df, sampleRate)
    feature_df.index.name = "TERMINALNO"

    feature_df.to_csv("train_feature.csv", index=True)
